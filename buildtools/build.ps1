...
#
# Only EV88G73A consumes it -- its main.c prints it -- so it is gated on the
# configuration rather than stamped into an image that never mentions it.
$extraCcPre = @()

if ($Configuration -eq 'CK64MC105_EV88G73A' -and $BuildId -ne 'none') {
    ...
    @(
        ...
        '#ifndef EV88G73A_BUILD_ID_H',
        '#define EV88G73A_BUILD_ID_H',
        "#define EV88G73A_BUILD_ID `"$effectiveBuildId`"",
        '#endif'
    ) -join "`r`n" | Set-Content -LiteralPath (Join-Path $genDir 'ev88g73a_build_id.h') -Encoding ascii

    # Forward slashes: this string is handed to make, which treats a backslash as
    # an escape.
    $extraCcPre += "-I`"$($genDir -replace '\\', '/')`""
    $extraCcPre += '-DEV88G73A_HAVE_BUILD_ID_H=1'
}
