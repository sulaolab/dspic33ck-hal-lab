...
$objectRoot = Join-Path $projectRoot 'build\tdm-feasibility'
New-Item -ItemType Directory -Force $objectRoot | Out-Null

$sourceFile = Join-Path $sourceRoot 'spikes\ck_spi_tdm_feasibility_probe.c'
$objectFile = Join-Path $objectRoot 'ck_spi_tdm_feasibility_probe.o'
...
Invoke-CheckedCommand -Description 'Compile CK SPI TDM feasibility probe' -Command {
    & $gcc $sourceFile `
        -o $objectFile `
        -c `
        -mcpu=33CK256MP508 `
