...
# OUTPUT LEVEL TRIM -- a deliberate -6.0 dB loudness reduction asked for after a
# listen (2026-08-29), applied on top of the model's AK-parity calibration rather
# than inside it. M.OUT_GAIN_Q15 keeps meaning "AK parity, 0.9 full scale peak" on
# its own, which is what the design-point comment's dB-below-signal ratio and the
# AK comparison are measured against; this scales it down at the very last step so
# a re-run of the model in isolation still reports the untrimmed parity number.
OUTPUT_TRIM_DB = -6.0
OUT_GAIN_Q15_SHIPPED = int(round(M.OUT_GAIN_Q15 * (10.0 ** (OUTPUT_TRIM_DB / 20.0))))
...
""" % (
    ...
    OUT_GAIN_Q15_SHIPPED, M.OUT_SHIFT,
...
print("  A_SCALE=%.1f  amp_q15 %d..%d  norm gain Q15=%d (AK-parity %d, trim %.1f dB)"
      % (M.A_SCALE, M.AMP_Q15.min(), M.AMP_Q15.max(), OUT_GAIN_Q15_SHIPPED,
         M.OUT_GAIN_Q15, OUTPUT_TRIM_DB))
