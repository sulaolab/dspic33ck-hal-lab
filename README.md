...
switch re-seeds every oscillator, which mid-sound is a click. A single-voice image
still binds both letters, and the absent one prints how to build an image holding
both (`-Define AVAS_CK_VOICE_BOTH=1`; on this 64 KB part that trades away the TDM
diagnostic commands, and the build says so if you ask for both).

For anything scripted, use the explicit forms rather than a toggle:

| command | effect |
| --- | --- |
| `*ta0001` / `*ta0000` | enter / leave the AVAS path |
| `*tv0000` / `*tv0001` | select `type_ty` / `type_lb` without starting it |
| `?tv` | which voice is loaded, whether it is sounding, and which voices this image contains |
| `?ta` | time the synth in the foreground, whole and per part (safe — no audio) |
| `*tb000f` | which parts run: 1 = carriers, 2 = envelope, 4 = gate, 8 = noise; `?tb` reports |
...
| `?tq` | the TDM1 load line — where the 43.0 % figure above comes from |
| `*ts` | analog-mute the codec and halt TDM/DMA. **Do this before flashing** |

The full letter map, and why the trap commands sit on `x` rather than `t`, is at the
top of [`src/uart_app/console_dispatch.c`](src/uart_app/console_dispatch.c).
