...
/*
 * boards/ev88g73a/board_profile.h -- THIS PROFILE'S ANSWERS, and nothing else.
 *
 ...
 * WHAT DOES NOT
 * -------------
 * Pins (ev88g73a_pins.h), rates and clock policy (ev88g73a_board.h), and anything
 * about the PART (that is the HAL's device tables). If a value here would change when
 * the part changed, it is in the wrong file.
 *
 * WHY IT EXISTS AT ALL (2026-08-05): this board used to gate the shared demos with
 * EV88G73A_ENABLE_* in its own main.c while DM330030 gated THE SAME shared modules with
 * ENA_* in app/app_config.h. Two vocabularies for one decision, and they collided: a
 * TDM loopback build needed BOTH `-DEV88G73A_ENABLE_TDM_LOOPBACK=1` (so this profile built
 * its port struct and calls) AND `-DENA_TDM_MASTER_LOOPBACK=1` (so the shared module's body
 * was not #if'd away), because that module reads app_config.h directly and cannot see a
 * #define made in main.c. Half-right meant a silent empty object and a link error. Now
 * there is one name per decision, resolved here, and app_config.h refuses anything it
 * cannot resolve.
 */
 ...
 */
#if !defined(DSPIC33CK_BOARD_EV88G73A)
#error "boards/ev88g73a/board_profile.h was included without DSPIC33CK_BOARD_EV88G73A -- the configuration's extra-include-directories and its preprocessor-macros disagree (firmware.X/nbproject/configurations.xml)."
#endif
 ...
 * and RX on RP59 (ev88g73a_pins.h). Capability, not choice -- so it is stated as a
