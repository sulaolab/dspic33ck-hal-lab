 ...
 * the accurate answer.
 *
 * It now owns ONE when WM8904 audio is enabled: 't' (audio transport path and load
 * controls). This is the hook working as intended -- the command attaches to a board
 * file instead of teaching the dispatcher about a board, and the handler itself lives
 * next to the config it controls (boards/ev88g73a/main.c) rather than here, because this
 * file knows about the console and nothing about TDM.
 *
