#ifndef EV88G73A_PINS_H
#define EV88G73A_PINS_H

/*
 * ev88g73a_pins.h -- every pin this board uses, and nothing else.
 *
 * WHY A SEPARATE HEADER
 * --------------------
 * These macros used to sit inside board_ev88g73a.c, where nothing outside that file
 * could name them. The cost showed up immediately: the WM8904 audio port in main.c
 * describes its wiring for the boot report, and with the numbers unreachable that
 * description was typed out by hand -- two copies of the same four RP numbers, one of
 * which would eventually be updated alone. The pinout is also the first thing a reader
 * wants when a jumper looks wrong, and it should not require reading code.
 *
 * DM330030 has had this file (as board_pins.h) since its own reorganisation; this is the
 * same idea under this board's prefix.
 *
 ...
 * to them (ev88g73a_board.{c,h}). This file answers exactly one question -- which pin --
 * so that a wiring change touches one place and a policy change never touches it.
 */

#include "nora_gpio.h"

/* ---- On-board LED and button (Curiosity Nano, DS70005517B) ---- */
#define EV88G73A_LED0_PIN     NORA_GPIO_PIN(NORA_GPIO_PORT_D, 10u)

/* Active-low: the button ties RD13 to GND and nothing else pulls it, so the internal
 * pull-up is load-bearing (DS70005517B Sec.4.2.2, "Mechanical Switch"). */
#define EV88G73A_SW0_PIN      NORA_GPIO_PIN(NORA_GPIO_PORT_D, 13u)

/* ---- Console UART1, through the Nano's on-board debugger CDC ---- */
#define EV88G73A_UART1_TX_RP  ((nora_gpio_rp_t)58u) /* RC10 */
#define EV88G73A_UART1_RX_RP  ((nora_gpio_rp_t)59u) /* RC11 */
 ...
 */
#define EV88G73A_I2C1_RP_ASDA ((nora_gpio_rp_t)56u) /* RC8 */
#define EV88G73A_I2C1_RP_ASCL ((nora_gpio_rp_t)57u) /* RC9 */
 ...
 * side drives BCLK/FS and is decided per build, not here -- see ev88g73a_tdm_pins_init().
 */
#define EV88G73A_TDM_RP_BCLK  ((nora_gpio_rp_t)50u) /* SCK1 */
#define EV88G73A_TDM_RP_FS    ((nora_gpio_rp_t)51u) /* SS1, CLC1OUT when master */
#define EV88G73A_TDM_RP_SDO   ((nora_gpio_rp_t)48u) /* SDO1, dsPIC -> codec DAC */
#define EV88G73A_TDM_RP_SDI   ((nora_gpio_rp_t)49u) /* SDI1, codec ADC -> dsPIC */
 ...
 * contradicted five other files in the same HEAD (wm8904_audio.c/.h, ev88g73a_board.c/.h,
 ...
 *     vocabulary that does not apply to CK -- on CK64MC105/CK256MP508 the bit is a BRG
 ...
 * measured.
 *
 * The clause follows EV88G73A_WM8904_DSPIC_IS_MASTER, the same switch main.c uses to pick
 * the role.
 *
...
#if ( EV88G73A_WM8904_DSPIC_IS_MASTER != 0 )
#define EV88G73A_AUDIO_WIRING_CLOCK_STR " dsPIC drives BCLK/FS (jumper A-extMCLK)"
#else
#define EV88G73A_AUDIO_WIRING_CLOCK_STR " codec drives BCLK/FS from X1 (jumper A-XTAL)"
#endif

#define EV88G73A_AUDIO_WIRING_STR                                              \
    "EV88G73A direct jumpers: BCLK=RP50 FS=RP51 SDO=RP48 SDI=RP49,"            \
    " I2C1 ASDA1=RP56 ASCL1=RP57," EV88G73A_AUDIO_WIRING_CLOCK_STR
 ...
 * EV88G73A_WM8904_DSPIC_IS_MASTER, and it was a board claim besides -- plan doc 20.4.)
 *
 ...
 */
#define EV88G73A_TDM_LOOPBACK_WIRING_STR                                       \
    "EV88G73A SPI1 master: scope BCLK=RP50(RC2) FS=RP51(RC3)"                  \
    " SDO=RP48(RC0); optional jumper SDO=RP48 -> SDI=RP49(RC1) to echo"

#endif /* EV88G73A_PINS_H */
