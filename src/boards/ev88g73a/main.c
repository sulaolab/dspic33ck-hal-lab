/*
 * dsPIC33CK64MC105 / EV88G73A first runnable profile.
 *
 * The EV88G73A board is the dsPIC33CK64MC105 Curiosity Nano. This profile uses
...
#include "ev88g73a_board.h"   /* bring-up + the operating point it reached */
#include "ev88g73a_io.h"      /* LED0, SW0 */
 ...
 * board no longer has a UART API of its own to call -- see ev88g73a_board.h. */
 ...
 * buildtools/build.ps1, which also defines EV88G73A_HAVE_BUILD_ID_H. Absent for
...
#error "boards/ev88g73a/main.c is EV88G73A-owned. Build it only in the CK64MC105_EV88G73A configuration -- if it reached another one, fix the <item ex=...> exclusions in firmware.X/nbproject/configurations.xml."
#endif
 ...
 * Shared -- this was ev88g73a_console.h, whose contents needed no board knowledge at all. */
 ...
 * reset-cause function of its own any more (see ev88g73a_board.h). */
#include "app_traps.h"

/* -------------------------------------------------------------------------- */
/* WHICH EXERCISERS THIS PROFILE RUNS -- ANSWERED IN board_profile.h.           */
/*                                                                            */
/* The values used to be defined here as EV88G73A_ENABLE_*, while DM330030      */
...
/* boards/ev88g73a/board_profile.h, and app/app_config.h refuses anything        */
 ...
 * SLAVE; EV88G73A_WM8904_DSPIC_IS_MASTER=1 swaps that (see the note beside the switch
 ...
 * ev88g73a_tdm_pins_init()/ev88g73a_i2c1_init() from ev88g73a_board.h.
 *
 ...
 *
 *   EV88G73A_ENABLE_SPI_DMA_MIN (deleted 2026-08-01) -- stage A of the SPI/DMA core
 *     rebuild: minimal 16-bit unframed SPI + DMA poking registers directly, to isolate
 *     faults before the transport core existed.
 *   EV88G73A_ENABLE_TDM_LOOPBACK (deleted 2026-08-02) -- TDM8/32-bit framed master
 ...
 * as EV88G73A_ENABLE_SPI_EXERCISER, by the same expression; it moved because the reason it is
 ...
 * ev88g73a_i2c1_init(), which the WM8904 path also calls -- same bus, same pins.
 *
...
#define EV88G73A_SELFTEST_DMA_CH (NORA_DMA_CHANNEL_3)
#endif
 ...
 *   - the bus bring-up, which is the board's (pins/pull-ups: see ev88g73a_board.c).
 ...
 * EV88G73A_ENABLE_TDM_LOOPBACK for this file. A -D that set only one produced an empty
 ...
 *
 *     buildtools\build.ps1 -Configuration CK64MC105_EV88G73A -Full `
 ...
 * The same routing function the WM8904 path uses, in the other role -- ev88g73a_tdm_pins_init
...
static const demo_tdm_master_loopback_port_t s_tdm_loopback_port = {
    .configure_pins = ev88g73a_tdm_pins_init,
    .wiring         = EV88G73A_TDM_LOOPBACK_WIRING_STR,
};
#endif
 ...
 * has no MCLK output at all; see the note in ev88g73a_pins.h beside the wiring string).
 * What the A jumper physically switches is a board fact this firmware does not set and
 * has not measured: plan doc 20.4.
 */
#ifndef EV88G73A_WM8904_DSPIC_IS_MASTER
#define EV88G73A_WM8904_DSPIC_IS_MASTER 0
#endif

static const wm8904_audio_port_t s_wm8904_port = {
    .configure_pins      = ev88g73a_tdm_pins_init,
    .i2c_init            = ev88g73a_i2c1_init,      /* same bus and pins as the I2C1 probe */
     ...
     * this tree still lacks is only an MCLK STAGE, in either position. See ev88g73a_board.h
     * and plan doc 20.4 + 21. */
    .mute_button_pressed = ev88g73a_sw0_pressed,    /* SW0, active-low, read raw */
    /* From ev88g73a_pins.h, beside the RP numbers it describes. This line used to be a
     * hand-typed second copy of them. */
    .wiring              = EV88G73A_AUDIO_WIRING_STR,
};

static const wm8904_audio_config_t s_wm8904_audio = {
    .port            = &s_wm8904_port,
    .i2c_inst_legacy = 1u,     /* wm8904.c maps this to NORA_I2C_INST_1 */
    .dspic_is_master = (EV88G73A_WM8904_DSPIC_IS_MASTER != 0),
     ...
     * to EV88G73A_STATUS_PRINT_PERIOD_MS, and two periods stacked on one line means the
 ...
 * console_board_onmsg() in ev88g73a_console_out.c.
 *
 ...
 *   ?ta  time the AVAS synth in the foreground, whole + per part (safe: no audio)
 *   *tb000f which AVAS parts run: 1=carriers 2=envelope 4=gate 8=noise; ?tb reports
 *   *ta0001 select the AVAS path / *ta0000 leave it -- NOT a *tp position, because
 *        an overrun there re-enters the block ISR forever and kills the console
 ...
 *
 * THREE OF THESE ARE OPTIONAL: ?ta, *tb/?tb and *tl are gated by
 ...
 * reason (a "not built" reply is a string). ?ta answers ERR_UNSUPPORTED instead, because
 * the letter 'a' is still here for *ta0000 and *ta0001. Everything else in the list above is
 * present in every image, deliberately: see the macro's comment for which and why.
 */
void ev88g73a_transport_console_onmsg(app_console_msg_t *msg);
...
void ev88g73a_transport_console_onmsg(app_console_msg_t *msg)
{
    ...
    /*
     * 'a' -- the AVAS synth. `?ta` times the engine in the foreground; `*ta0001`
     * STARTS the synth and `*ta0000` STOPS it, which fades out over ~3 s rather than
     * cutting -- it is a stage of the chain path now, not a path of its own (see
     * wm8904_audio.h). Same explicit-value grammar as *tq below, for the same reason --
     * a bare *ta must not mean "whichever state you are not in" when one of those
     * states can hang the console.
     */
    if (msg->name == 'a') {
        if (msg->kind == '?') {
#if WM8904_AUDIO_ENABLE_TDM_DIAG
            if (msg->data_len != 0u) {
                console_out_str(" ?ta: takes no value\n");
                msg->status = APP_CONSOLE_ERR_BAD_PARM_LEN;
                return;
            }
            wm8904_audio_avas_bench();
            msg->status = APP_CONSOLE_OK;
#else
            /* The bench is not in this image (WM8904_AUDIO_ENABLE_TDM_DIAG=0). The status
             * code alone, with no line of its own: a "not built" reply is a string, and the
             * string is what the gate exists to buy back. *ta itself is unaffected -- the
             * letter is here, only its query form is gone, which is why this cannot simply
             * fall through to ERR_NOT_FOUND the way *tb and *tl do. */
...

        {
            ...
            if ((msg->data_len == 0u) || (msg->data_len > 2u)) {
                console_out_str(" *ta: needs a value -- *ta0000 = stop (~3 s fade),"
                                " *ta0001 = start (measure with ?ta first)\n");
                msg->status = APP_CONSOLE_ERR_BAD_PARM_LEN;
                return;
            }
            ...
            if (value > 1u) {
                console_out_str(" *ta: value must be 0000 or 0001\n");
                msg->status = APP_CONSOLE_ERR_BAD_DATA;
                return;
            }

            wm8904_audio_avas_enable_set(value != 0u);
        }
        msg->status = APP_CONSOLE_OK;
        return;
    }
    ...
    /*
     * 'v' -- WHICH AVAS VOICE. `?tv` reports the loaded voice, whether it is sounding
     * and which voices this image contains; `*tv0000` selects Type_TY L1 and `*tv0001`
     * the Type_LB L3, WITHOUT starting either -- that is what the 'a' / 'A' hotkeys
     * are for, and keeping the two apart is what makes this form usable from a script
     * where "and also start making noise" would be a surprise.
     *
     * Explicit value, no bare form, for the same reason as *ta and *tq: "whichever one
     * you are not on" is the wrong thing to send when the answer depends on a state the
     * sender cannot see. It refuses while the engine still owns its oscillators (fade
     * included) and prints why -- see wm8904_audio_avas_voice_set().
     */
    if (msg->name == 'v') {
        if (msg->kind == '?') {
            if (msg->data_len != 0u) {
                console_out_str(" ?tv: takes no value\n");
                msg->status = APP_CONSOLE_ERR_BAD_PARM_LEN;
                return;
            }
            wm8904_audio_avas_voice_report();
            msg->status = APP_CONSOLE_OK;
            return;
        }

        {
            ...
            if ((msg->data_len == 0u) || (msg->data_len > 2u)) {
                console_out_str(" *tv: needs a value -- *tv0000 = type_ty,"
                                " *tv0001 = type_lb (selects only; a / A also start)\n");
                msg->status = APP_CONSOLE_ERR_BAD_PARM_LEN;
                return;
            }
            ...
            if (value > 1u) {
                console_out_str(" *tv: value must be 0000 (type_ty) or 0001 (type_lb)\n");
                msg->status = APP_CONSOLE_ERR_BAD_DATA;
                return;
            }

            msg->status = wm8904_audio_avas_voice_set((uint8_t)value)
                              ? APP_CONSOLE_OK
                              : APP_CONSOLE_ERR_BAD_DATA;
        }
        return;
     ...
     *
     * No bare form: the same explicit-value rule as *tq, *ta and *tb. A bare *ti has no
    ...
    if (msg->name == 'q') {
        ...
        /*
         * *tq0000 / *tq0001 -- fleet grammar, the same shape as sonora's *ar0001: the hex
         * digits after the name are already decoded into msg->data[] by app_console.c.
         *
         * Accept a one- or two-byte payload (*tq00 and *tq0000 both mean off) because the
         * width is a typing convention, not information -- but REFUSE an empty payload
         * rather than treating it as a toggle. A bare *tq would then mean "whichever state
         * you are not in", which is exactly the ambiguity an explicit value exists to
         * remove, and it is the form most likely to be typed by mistake.
         */
        {
            uint16_t value;

            if ((msg->data_len == 0u) || (msg->data_len > 2u)) {
                console_out_str(" *tq: needs a value -- *tq0000 = off, *tq0001 = on\n");
                msg->status = APP_CONSOLE_ERR_BAD_PARM_LEN;
                return;
            }
            ...
            if (value > 1u) {
                console_out_str(" *tq: value must be 0000 or 0001\n");
                msg->status = APP_CONSOLE_ERR_BAD_DATA;
                return;
            }
 ...
 * ev88g73a_console_out.c. Two keys, and they are ONE VOICE EACH rather than a key for
 ...
 *
 * IT IS A BARE TOGGLE, WHICH THE *ta GRAMMAR ABOVE DELIBERATELY REFUSES -- so the reason
 * that refusal existed has to be said, not skipped. It was written when entering this
 * path could hang the console: the engine cost 119 % of the block period, an overrun
 * stops the foreground that owns the console, and this board has no reset button, so
 * recovery is a reprogram. What changed is measured rather than assumed -- 289.2 us of a
 * 667 us block, 43.0 %, miss = 0, 382.2 us of margin. The explicit form stays exactly as it was for anything scripted, where
 * "whichever state you are not in" is still the wrong thing to send.
 *
 * The state comes from the audio module rather than a flag kept here, so the key and
 * *ta0001 cannot drift apart -- the same reason sonora's hotkey and its console command
 * share one enable-state source.
 */
bool ev88g73a_transport_console_hotkey(uint8_t ch)
{
    if (ch == (uint8_t)'a') {
 ...
 *
 *     actual Fcy = EV88G73A_ASSUMED_FCY_HZ * 10 / (seconds for 10 cycles)
 *
 ...
 */
#define EV88G73A_ASSUMED_FCY_HZ (EV88G73A_TARGET_FOSC_HZ / 2UL)  /* 100 MHz */

/* 500 ms per LED state -> 1.000 s full cycle. */
#define EV88G73A_LED_STATE_PERIOD_MS (500u)
 ...
 */
#define EV88G73A_SW0_SAMPLE_PERIOD_MS (100u)
...
#define EV88G73A_AUDIO_OBSERVE_PERIOD_MS (1u)
 ...
 * EV88G73A_STATUS_PRINT_PERIOD_MS / (periods_per_LED_state * 100), i.e. the print period
 ...
 * never was).
 */
#define EV88G73A_STATUS_PRINT_PERIOD_MS (2000u)
 ...
 */
#if !DEMO_ENABLE_WM8904_AUDIO
#define EV88G73A_SW0_PAUSES_BLINK 1
#else
#define EV88G73A_SW0_PAUSES_BLINK 0
#endif

#if EV88G73A_SW0_PAUSES_BLINK
/*
...
static void sw0_latch_poll(void)
{
    ...
    bool        pressed = ev88g73a_sw0_pressed();
...
#endif /* EV88G73A_SW0_PAUSES_BLINK */
 ...
 * construction (EV88G73A_SW0_PAUSES_BLINK is !DEMO_ENABLE_WM8904_AUDIO), so this never
 ...
 */
#if EV88G73A_SW0_PAUSES_BLINK || DEMO_ENABLE_WM8904_AUDIO
#define EV88G73A_SW0_SAMPLED_IN_WAIT 1
#else
#define EV88G73A_SW0_SAMPLED_IN_WAIT 0
#endif
...
static void wait_next_led_edge(void)
{
    ...
    static bool     seeded;
#if EV88G73A_SW0_SAMPLED_IN_WAIT
    ...
    while ((uint32_t)(GetTicks() - s_led_edge_ms) < EV88G73A_LED_STATE_PERIOD_MS) {
        /*
        ...
        if ((uint32_t)(GetTicks() - audio_observe_ms) >=
            EV88G73A_AUDIO_OBSERVE_PERIOD_MS) {
...
#if EV88G73A_SW0_SAMPLED_IN_WAIT
        if ((uint32_t)(GetTicks() - sw0_sample_ms) >= EV88G73A_SW0_SAMPLE_PERIOD_MS) {
            sw0_sample_ms += EV88G73A_SW0_SAMPLE_PERIOD_MS;
#if EV88G73A_SW0_PAUSES_BLINK
            sw0_latch_poll();
#else
    ...
    s_led_edge_ms += EV88G73A_LED_STATE_PERIOD_MS;
}

static void report_operating_point(void)
{
    console_out_str(
        "\nEV88G73A dsPIC33CK64MC105 bring-up: LED0/UART alive\n");
     ...
     * places DM330030 keeps them. The ev88g73a_fosc_hz()/_clock_at_target()/... wrappers
     * this used to call are gone; see ev88g73a_board.h. */
    console_out_str("Fosc Hz  = ");
    console_out_u32(nora_clock_get_fosc_hz());
    console_out_str("\nFcy Hz   = ");
    console_out_u32(nora_clock_get_fcy_hz());
    console_out_str("\nBaud     = ");
    console_out_u32(nora_uart_get_baudrate(EV88G73A_CONSOLE_UART_INST));
    console_out_str("\nClock    = ");
    console_out_str(
        g_ev88g73a_clock_on_target ? "PLL at target\n"
                                   : "NOT at target (fell back to FRC)\n");
    console_out_str("PLLstatus= ");
    console_out_u32((uint32_t)g_ev88g73a_clock_init_status);
    console_out_str("  (0 = OK, 4 = timeout)\nPLLdiag  = ");
    console_out_u32((uint32_t)g_ev88g73a_clock_diag);
     ...
     * the actual divider chain, and what showed a comment in ev88g73a_board.c had been
    ...
    console_out_str("\nCOSC@sw  = ");
    console_out_u32(
        ((uint32_t)g_ev88g73a_osccon_after_switch >> 12) & 0x7u);
    ...
    console_out_u32(
        ((uint32_t)g_ev88g73a_osccon_after_switch >> 5) & 0x1u);
    console_out_str("\n");
}
 ...
 * (g_ev88g73a_*_init_ok): profile_report() runs after the console exists, so a tick that
 ...
 */
static volatile bool g_ev88g73a_tick_start_ok;
...
void profile_bring_up(void)
{
     ...
     */
    g_ev88g73a_tick_start_ok = timer_app_start_from_fcy(EV88G73A_ASSUMED_FCY_HZ);
}
...
void profile_start(void)
{
     ...
     * already known. Those stages were deleted with ev88g73a_spi_dma_min.c, so
     ...
     */
    if (!dma_selftest_run(EV88G73A_SELFTEST_DMA_CH)) {
        console_out_str(
    ...
    console_out_str("      'a' / 'A' alone (no Enter) start-stop the AVAS synth's"
                    ...
                    " *ta0001 / *ta0000 and *tv0000 / *tv0001 do it explicitly\n");
#endif
}
 ...
 * than an intention -- see EV88G73A_STATUS_PRINT_PERIOD_MS.
 */
bool profile_wait_next_tick(void)
{
    ...
    if (blink_paused) {
        ...
        if (led_on) {
            led_on = false;
            ev88g73a_led0_set(false);
        }
    } else {
        led_on = !led_on;
        ev88g73a_led0_set(led_on);
    }
#else
    led_on = !led_on;
    ev88g73a_led0_set(led_on);
     ...
     */
    report = ((uint32_t)(GetTicks() - status_last_ms) >= EV88G73A_STATUS_PRINT_PERIOD_MS);
    if (report) {
