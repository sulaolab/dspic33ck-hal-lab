#ifndef EV88G73A_BOARD_H
#define EV88G73A_BOARD_H

/*
 * ev88g73a_board.{c,h} -- BRING-UP. Everything here runs once, in order, before the
 ...
 *
 * Was board_ev88g73a.{c,h}, which was all three roles in one file. The split is by WHEN
 * each part matters, because that is a line a later reader can re-derive:
 *
 *   ev88g73a_pins.h    facts      compile time only, no code
 *   ev88g73a_board.*   bring-up   once, in order   <- this file
 *   ev88g73a_io.*      runtime    every iteration, owns no init
 *
 * TWO THINGS LEFT THIS HEADER RATHER THAN MOVING WITHIN IT:
 *   - LED0 and SW0 accessors      -> ev88g73a_io.h  (runtime, not bring-up)
 *   - the exerciser switches      -> board_profile.h (2026-08-05; they were
 *     EV88G73A_ENABLE_* in main.c until then). They select which SHARED app/ modules this
 *     PROFILE runs; nothing about the board changes with them. The stop in main.c was
 *     half-way: DM330030 answered the same questions as ENA_* in app/app_config.h, so one
 *     decision had two names and a TDM build had to pass both. Now each board directory
 *     has a board_profile.h with one normalized name per decision, app/app_config.h
 *     refuses anything unresolved, and main.c only READS them.
 *
 * The pin configuration this file performs is still what owns direction on those pins:
 * ev88g73a_io.c only reads and writes levels.
 */

#include <stdbool.h>
#include <stdint.h>

/* For NORA_UART_INST_1, named by EV88G73A_CONSOLE_UART_INST below. Included
 * here rather than left to each caller so the constant is usable on its own. */
#include "nora_uart.h"

/* For nora_spi_i2s_tdm_clock_role_t in ev88g73a_tdm_pins_init()'s signature. */
#include "nora_spi_i2s_tdm.h"

/* For nora_clock_status_t on g_ev88g73a_clock_init_status below, and because
...
#define EV88G73A_FRC_HZ         (8000000UL)     /* internal FRC, PLL input */
#define EV88G73A_TARGET_FOSC_HZ (200000000UL)   /* device max: 200 MHz Fosc / 100 MIPS */
 ...
 */
#define EV88G73A_UART_BAUD_FAST (230400UL)
#define EV88G73A_UART_BAUD_SAFE (9600UL)

/*
 * WHICH UART is the console, as a value rather than a set of wrapper functions.
 *
 * This board used to export seven ev88g73a_uart_*() functions. Four of them had
 * exactly one caller each -- ev88g73a_console_out.c -- and the other three were
 ...
 * transport now lives entirely in ev88g73a_console_out.c on top of hal_uart, and
 ...
 * once, here, and used by the two files that need it: ev88g73a_console_out.c (the
 * transport) and ev88g73a_board.c (the peripheral bring-up). main.c reads the baud
 * it actually got from the HAL with it.
 */
#define EV88G73A_CONSOLE_UART_INST (NORA_UART_INST_1)
 ...
 * through main.c as EV88G73A_ENABLE_* in between) -- see the note at the top of this file.
 */
 ...
 *         ev88g73a_fosc_hz()/_fcy_hz() only forwarded it, which made a board look like
 ...
 *     g_ev88g73a_clock_on_target                      false means the FRC->PLL bring-up
 *         did not reach EV88G73A_TARGET_FOSC_HZ and the board is still on the 8 MHz FRC.
 ...
 *     g_ev88g73a_osccon_after_switch                  OSCCON as it read immediately
 ...
 *     g_ev88g73a_clock_init_status                    the HAL status for the request.
 *     g_ev88g73a_clock_diag                           nora_clock_dspic33ck_diag_t: WHICH
 ...
 *
 * All four volatiles are defined in ev88g73a_board.c beside the stage that sets them,
 * and are volatile precisely so a debugger can read them on a board whose console never
 * came up.
 */

/*
 * There is deliberately no ev88g73a_uart_baud(). It returned a static this file's .c
 * had set from EV88G73A_UART_BAUD_FAST/SAFE, i.e. what the board ASKED for; the UART
 * HAL records what it actually applied. Ask the HAL:
 *
 *     nora_uart_get_baudrate(EV88G73A_CONSOLE_UART_INST)
 *
...
extern volatile nora_clock_status_t g_ev88g73a_clock_init_status;
extern volatile bool                     g_ev88g73a_clock_on_target;
extern volatile uint16_t                 g_ev88g73a_osccon_after_switch;
extern volatile uint16_t                 g_ev88g73a_clock_diag;

/*
 * LED0 AND SW0 ARE NOT HERE, they are in ev88g73a_io.h. Their PIN CONFIGURATION still
 * happens in this file's ev88g73a_board_init(), and stays its exclusive business -- the
 ...
 * calls uart_platform/console_out.h, whose EV88G73A implementation is
 * ev88g73a_console_out.c talking to hal_uart directly.
 *
 ...
 * EV88G73A_CONSOLE_UART_INST above.
 */
 ...
 *
 * ev88g73a_reset_cause_str()/_raw() used to be declared here and the seam functions
 ...
 * ev88g73a_board_init() LATCHES RCON before anything else can touch it, and clears it, so
 ...
 *
 * Was ev88g73a_i2c1_init() in ev88g73a_i2c1_probe.c. It stayed on the board when the
