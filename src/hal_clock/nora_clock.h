 ...
 * input, VCO, divider, and output constraints from the dsPIC33AK512MPS512 DFP
 * 1.3.185 device facts, then drives the internal register layer that owns switch,
 * ready, and timeout sequencing.
 *
 ...
 *   name (nora_clock_dspic33ak_raw_capture()), so the register set it depends on
 ...
 *   remaining values name nodes of the AK fan-out tree (the fractional-divider
 *   VCO taps, the REFI input pins); they live in a separate numeric range and
 *   exist for nora_clock_pll_configure() and the AK CLKGEN calls.  A backend that
...
/*
 * Nominal FRC frequency used by this contract, in Hz -- 8 MHz on the dsPIC33AK parts
 * this backend builds for -- valid while the device's FRC tuning control is at its
 ...
 * contract -- no call in this HAL writes a tuning field, and no source in this project
 * writes one either, which is what makes the constant true here by construction rather
 * than by hope.
 *
