 ...
 *
 * MEASURED, not assumed: with the diag commands in, the BOTH=1 link came up 620 PC units
 * (930 B) short -- 82 unplaced sections and a "Could not allocate program memory" that names
 * no cause. An #error here costs nothing and names the knob.
 */
#if (AVAS_CK_VOICE_BOTH) && (WM8904_AUDIO_ENABLE_TDM_DIAG)
#error "AVAS_CK_VOICE_BOTH=1 does not fit CK64MC105 together with the TDM diag commands (measured 930 B short). Build it with WM8904_AUDIO_ENABLE_TDM_DIAG=0, which is what a -Define AVAS_CK_VOICE_BOTH=1 selects by itself."
#endif
...
/*
 * "Has the synth been started", the latch behind `a` and *ta -- and it is NOT derived
 ...
 * 32-bit gate the foreground writes (the torn-write lesson from set_parts), and it means
 * the ?ta bench -- which mutates the engine in the foreground -- can refuse on one byte.
 */
                       ...
                       nora_tdm_slot_t       *dst)
{
     ...
     *    itself, so the stage costs nothing once the tail is below -50 dB -- and the
     *    engine is then untouched by this priority, which is what lets ?ta measure it in
     *    the foreground without racing.
     */
    if (s_chain_avas_run) {
        chain_stage_avas();
...
/* -------------------------------------------------------------------------- */
/* ?ta -- time the AVAS engine in the FOREGROUND.                              */
/*                                                                            */
/* This exists because the obvious way to find out what the engine costs -- put */
/* it in the block callback and read the load monitor -- destroyed the board's   */
/* console the first time it was tried. When the block ISR takes longer than the */
/* block period, the next DMA interrupt is already pending when it returns, so    */
/* the ISR re-enters forever and the foreground NEVER runs: no load line, no      */
/* command echo, no trap report, nothing. The load monitor cannot measure an      */
/* overrun because the thing that prints it does not get to run. Recovery needed  */
/* a reprogram, since this kit has no reset button.                             */
/*                                                                            */
/* So the cost is measured here instead, off the ISR entirely. Same arithmetic,  */
/* same state, same optimisation level; only the caller differs. It cannot        */
/* overrun anything and it emits no audio.                                      */
/*                                                                            */
/* Refuses while the AVAS stage is running -- started, or still fading out: it    */
/* would advance the same oscillator state the block ISR is reading, so the       */
/* numbers AND the audio would be wrong. Being ON THE CHAIN PATH is not a reason  */
/* to refuse (it is the default path now); the chain skips the stage entirely once */
/* the fade has finished, and then nothing but this function touches the engine.   */
/* -------------------------------------------------------------------------- */
#define WM8904_AUDIO_AVAS_BENCH_SAMPLES  320u   /* 10 envelope rebuilds' worth */
#define WM8904_AUDIO_AVAS_BENCH_REBUILDS  10u

/* us_x10 -> the per-32-frame-block figure, and that as a percent of the nominal
 * 666.7 us block at the coefficient table's 48 kHz. Printed as one helper because
 * the interesting number is always "what would this add to the block ISR". */
static void avas_bench_line(const char *what, uint32_t us_x10_per_block)
{
    console_out_str("      ");
    console_out_str(what);
    console_out_str(" ");
    console_out_u32(us_x10_per_block / 10u);
    console_out_str(".");
    console_out_u32(us_x10_per_block % 10u);
    console_out_str("us/block = ");
    console_out_u32(us_x10_per_block * 100u / 6667u);
    console_out_str("%\n");
}
...
void wm8904_audio_avas_bench(void)
{
    ...
    if (s_chain_avas_run || avas_line_ck_is_active(&s_avas)) {
        console_out_str(" ?ta: not while the avas synth is sounding -- stop it (a, or"
                        " *ta0000) and let the ~3 s fade finish\n");
        return;
    }
    if (!nora_high_res_timer_is_initialized()) {
        console_out_str(" ?ta: high-res timer not initialised -- no time base\n");
        return;
    }

    saved_parts = s_avas.parts;

    /* --- whole engine, exactly what the block callback would call ------------ *
     * The engine returns 0 without touching an oscillator while fully gated off,
     * so a benchmark with the gate shut would time the early-out. Open it. */
    avas_line_ck_gate_on(&s_avas);
    avas_line_ck_set_parts(&s_avas, AVAS_TYPE_TY_CK_PART_ALL);
    s_avas.gate = 0x7FFFFFFFL;          /* skip the 4 s ramp; cost is independent of it */
    start = nora_high_res_timer_get_count();
    for (i = 0u; i < WM8904_AUDIO_AVAS_BENCH_SAMPLES; i++) {
        sink += (int32_t)avas_line_ck_process_sample(&s_avas);
    }
    full_x10 = nora_high_res_timer_elapsed_us_x10(start);

    /* --- carriers alone, at full rate --------------------------------------- */
    avas_line_ck_init(&s_avas);
    s_avas.gate = 0x7FFFFFFFL;
    start = nora_high_res_timer_get_count();
    for (i = 0u; i < WM8904_AUDIO_AVAS_BENCH_SAMPLES; i++) {
        sink += avas_line_ck_carriers_only(&s_avas);
    }
    carr_x10 = nora_high_res_timer_elapsed_us_x10(start);

    /* --- one envelope rebuild = 185 baseband oscillators -------------------- *
     * This happens ONCE per DEC samples, and DEC is the block size, so one
     * rebuild is exactly one block's worth of it. */
    avas_line_ck_init(&s_avas);
    start = nora_high_res_timer_get_count();
    for (i = 0u; i < WM8904_AUDIO_AVAS_BENCH_REBUILDS; i++) {
        avas_line_ck_rebuild_only(&s_avas);
    }
    reb_x10 = nora_high_res_timer_elapsed_us_x10(start);

    avas_line_ck_init(&s_avas);                       /* closed gate, measured phases */
    avas_line_ck_set_parts(&s_avas, saved_parts);

    console_out_str(" ?ta: AVAS cost per 32-frame block, timed in the foreground\n");
    avas_bench_line("whole engine    ",
                    full_x10 * 32u / WM8904_AUDIO_AVAS_BENCH_SAMPLES);
    avas_bench_line("carriers (x32)  ",
                    carr_x10 * 32u / WM8904_AUDIO_AVAS_BENCH_SAMPLES);
    avas_bench_line("envelope (x1)   ",
                    reb_x10 / WM8904_AUDIO_AVAS_BENCH_REBUILDS);
    console_out_str("      parts mask now ");
    console_out_u32((uint32_t)saved_parts);
    console_out_str(" (" AVAS_TYPE_TY_CK_PART_HELP "), acc=");
    console_out_u32((uint32_t)avas_line_ck_acc_mode());
    /* The phase widths, for the same reason acc= is printed: sink= is only
     * comparable between images that computed it the same way, and a narrower
     * phase changes the line frequencies, so it changes sink= legitimately.
     * Taken straight from the macros -- unlike ACC_MODE, which is chosen inside
     * avas_synth_line_ck.c, these are settled in the generated table header this file
     * already sees, so no accessor is needed. */
    console_out_str(", ph=");
    console_out_u32((uint32_t)AVAS_TYPE_TY_CK_PHBITS_CAR);
    console_out_str("/");
    console_out_u32((uint32_t)AVAS_TYPE_TY_CK_PHBITS_BB);
    /* And which coordinates the envelope was interpolated in, for the third time the
     * same reason applies: polar is a different arithmetic, so it has its own sink=
     * and its own two timings, and a log of those numbers is ambiguous without it. */
    console_out_str(", env=");
    console_out_str(((AVAS_TYPE_TY_CK_ENVINTERP) == 1) ? "polar" : "rect");
    console_out_str(", sink=");
    console_out_u32((uint32_t)sink);
    console_out_str("\n");

#if defined(AVAS_TYPE_TY_CK_SATPROBE)
    /* Printed as unsigned 16-bit so the two outcomes read as 32767 (SATDW
     * saturates a W-register destination) and 65530 (it wraps to -6). */
    console_out_str("      SATDW probe, sac -> W register: ");
    console_out_u32((uint32_t)(uint16_t)avas_line_ck_satprobe());
    console_out_str(" (32767=saturates 65530=wraps)\n");
#endif
}
...
/* off the cheapest way to HEAR what the 185 lines contribute, and because the      */
/* per-part split is what ?ta attributes the cost with.                            */
/*                                                                            */
...
#endif /* WM8904_AUDIO_ENABLE_TDM_DIAG -- ?ta and *tb */
...
void wm8904_audio_start(const wm8904_audio_config_t *cfg)
{
     ...
     * and `a` (or *ta0001) is what starts it. */
...
bool wm8904_audio_path_set(uint16_t index)
{
     ...
     * separate command that carries the warning (see wm8904_audio_avas_enable_set).
     */
...
/* -------------------------------------------------------------------------- */
/* `a` / *ta0001 / *ta0000 -- START and STOP the synth. Not a path any more.    */
/*                                                                            */
...
void wm8904_audio_avas_enable_set(bool on)
{
    if (!on) {
        if (!s_avas_on && !avas_line_ck_is_active(&s_avas)) {
            console_out_str(" *ta: avas synth already stopped\n");
            return;
        }
        ...
        console_out_str(((s_avas.parts & AVAS_TYPE_TY_CK_PART_GATE) != 0u)
                            ? " *ta: avas synth stop -- fading out over ~3 s\n"
                            : " *ta: avas synth stop -- immediate, because *tb has the"
                              " gate part masked off (no fade exists to render)\n");
        return;
    }

    if (!s_avas_rate_ok) {
        console_out_str(" *ta: refused -- coefficient table is for another sample rate"
                        " (see bring-up log)\n");
        return;
    }

    if (s_avas_on) {
        console_out_str(" *ta: avas synth already started\n");
        return;
    }

    console_out_str(" *ta: avas synth start, voice = ");
    console_out_str(avas_line_ck_voice_name(wm8904_audio_avas_voice()));
...
/* -------------------------------------------------------------------------- */
/* `a` / `A` / *tv -- WHICH VOICE, and why the exclusivity lives here.          */
/*                                                                            */
...
bool wm8904_audio_avas_voice_set(uint8_t voice)
{
    if (!avas_line_ck_voice_present(voice)) {
        console_out_str(" *tv: refused -- this image holds only the ");
        console_out_str(avas_line_ck_voice_name(wm8904_audio_avas_voice()));
    ...
    if (avas_is_sounding()) {
        console_out_str(" *tv: refused -- the ");
        console_out_str(avas_line_ck_voice_name(wm8904_audio_avas_voice()));
        console_out_str(" voice is still sounding. Stop it first (a / *ta0000) and let"
    ...
    if (!avas_line_ck_voice_set(&s_avas, voice)) {
        ...
        console_out_str(" *tv: refused by the engine -- it is still sounding\n");
        return false;
    }

    console_out_str(" *tv: voice = ");
...
{
    if (avas_is_sounding()) {
        if (wm8904_audio_avas_voice() == voice) {
            wm8904_audio_avas_enable_set(false);
        } else {
            (void)wm8904_audio_avas_voice_set(voice);   /* prints the reason */
        }
        return;
    }

    if (!wm8904_audio_avas_voice_set(voice)) {
        return;                                        /* prints the reason */
    }
    wm8904_audio_avas_enable_set(true);
}
...
void wm8904_audio_avas_voice_report(void)
{
    ...
    console_out_str(" ?tv: voice = ");
    console_out_str(avas_line_ck_voice_name(wm8904_audio_avas_voice()));
    console_out_str(avas_is_sounding() ? " (sounding)\n" : " (silent)\n");

    console_out_str(" ?tv: resident =");
    for (v = 0u; v < (uint8_t)AVAS_LINE_CK_VOICE_COUNT; v++) {
        if (avas_line_ck_voice_present(v)) {
            console_out_str(" ");
            console_out_str(avas_line_ck_voice_name(v));
        }
    }
    console_out_str("   (a = type_ty, A = type_lb, exclusive)\n");
}
