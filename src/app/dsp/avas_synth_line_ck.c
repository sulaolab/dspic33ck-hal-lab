 ...
 *     an overrun at -Os.  That estimate HELD: the engine's measured increment is the
 *     ~289 us of a 667 us block that *ta warns about, so -Os would still overrun.
 ...
 * -------------------------------------
 * avas_line_ck_carriers_only() and avas_line_ck_rebuild_only() exist ONLY so
 * ?ta can time the two halves in the foreground.  Left inlinable, the compiler
 * gave each wrapper its OWN copy of the loop -- 654 bytes of duplicate on a part
 * with 3 % of its flash left, and, worse, ?ta was then timing code that was not
 * the code the block ISR runs.  Two copies can differ in register allocation,
 * and register allocation is exactly what this loop is sensitive to (see the
 * toolchain note in process_carriers).  A benchmark of a different build of the
 * same source is not a benchmark.
 *
 * So the two hot functions are noinline: one copy, and the ?ta figure is
 * measured on the instructions the ISR executes.  The cost is one call per
 * sample for the carriers, ~4 cycles of 2048.
 */
#if defined(__GNUC__)
 ...
 *     project -- A and B are not shadowed on interrupt entry, and ?ta runs these
 *     same functions in the FOREGROUND while the block ISR runs them -- is
 *     handled by the toolchain, not by us.  Verified in the listing.
 *
                                      ...
                                      int16_t *out_i, int16_t *out_q)
{
    ...
    for (i = 0u; i < n; i++) {
        ...
        *php++ = (avas_type_ty_ck_bbph_t)(ph + (avas_type_ty_ck_bbph_t)*stp++);
...
static void avas_line_ck_rebuild_envelope(avas_line_ck_t *s)
{
    ...
    for (k = 0u; k < set->clusters; k++) {
...
#if ((AVAS_TYPE_TY_CK_ENVINTERP) == 1)
        {
             ...
             * avoid costs nothing. */
            c->step_eff = (avas_type_ty_ck_carph_t)(set->car_step[k]
...
int32_t avas_line_ck_carriers_only(avas_line_ck_t *s)
{
    return avas_line_ck_process_carriers(s);
}


void avas_line_ck_rebuild_only(avas_line_ck_t *s)
{
    avas_line_ck_rebuild_envelope(s);
}
...
/*
 * Which accumulation this image was built with, so the console can say it.
 *
 * A function for a compile-time constant, because AVAS_TYPE_TY_CK_ACC_MODE is chosen in
 * this file and callers outside it would otherwise have to be told twice -- and a
 * second copy of "which mode is the default" is exactly the thing that goes stale.
 * Worth the handful of bytes: `?ta` is where the sink= checksum and the two halves
 * are read, and a log of those numbers is ambiguous unless it also says which
 * arithmetic produced them. That ambiguity was real -- three images were flashed in
 * one afternoon to close section 14's third link.
 */
uint8_t avas_line_ck_acc_mode(void)
{
    return (uint8_t)AVAS_TYPE_TY_CK_ACC_MODE;
}


/* ------------------------------------------------------------------------- *
 * SATDW probe -- present only in a -Define AVAS_TYPE_TY_CK_SATPROBE=1 build.
 *
 * The open question was whether SATDW = 1 (data-space write saturation) applies
 * when `sac`'s destination is a W register rather than memory.  The `sink=`
 * checksum CANNOT answer it, contrary to what section 13 first assumed: the
 * shipped table's peak leaves A[31:16] at 31 722, three per cent short of the
 * clamp, so modes 1 and 2 agree whatever the saturation does.  Nothing in the
 * engine reaches the boundary, so the boundary has to be driven deliberately.
 *
 * In fractional mode each mac adds (a*b) << 1, so two of 32767 * 32767 leave
 * A = 4 294 705 156 -- still far inside 40 bits, which is the point: this tests
 * the STORE, not accumulator wrap.  `sac A, #0` stores A[31:16] = 65 530, which
 * does not fit the signed 16-bit destination:
 *
 *     saturating -> +32767      wrapping -> -6 (0xFFFA)
 *
 * The operands are volatile so the sum happens on silicon instead of being
 * folded by the compiler.
 * ------------------------------------------------------------------------- */
#if defined(AVAS_TYPE_TY_CK_SATPROBE)
#if (AVAS_TYPE_TY_CK_ACC_MODE != 2)
#error "AVAS_TYPE_TY_CK_SATPROBE probes the mac/sac path -- build it with AVAS_TYPE_TY_CK_ACC_MODE=2."
#endif
int16_t avas_line_ck_satprobe(void)
{
    volatile register int16_t acc asm("A");
    volatile int16_t a = 32767;
    volatile int16_t b = 32767;

    acc = __builtin_clr();
    acc = __builtin_mac_16(acc, a, b);
    acc = __builtin_mac_16(acc, a, b);

    return __builtin_sac_16(acc, 0);
}
#endif
 ...
 *
 * voice_present() answers false for EVERY voice, so `*tv` refuses with a reason
 * instead of appearing to select something.  That is the same contract the
