 ...
 *
 * The mask is kept for the two things it still does, both of which are why the
 * cost is known rather than assumed: it produces the per-part breakdown `?ta`
 * prints, and switching ENVELOPE on and off is how the 185 lines' contribution is
 * HEARD rather than argued about. It is no longer a way to make the engine fit.
 *
...
/* -------------------------------------------------------------------------
 * The two halves, exposed individually SO THEY CAN BE TIMED SEPARATELY.
 *
 * Normally these would stay private. They are public because the load monitor
 * cannot measure an overrun (it is printed by the foreground, which an overrun
 * stops) and a hand-count of the inner loops came out 2.5x optimistic -- so the
 * only way to learn where the cost is, is to time each half in the foreground.
 * That was first needed when the engine did not fit at all; it is still the only
 * way to attribute the cost now that it does. See wm8904_audio.c's ?ta.
 *
 * They advance real oscillator state, so a caller that is not the block path must
 * re-init afterwards.
 * ------------------------------------------------------------------------- */
int32_t avas_line_ck_carriers_only(avas_line_ck_t *s);
void    avas_line_ck_rebuild_only(avas_line_ck_t *s);

/* Which carrier/envelope accumulation this image was built with: 0 legacy int32 with
 * TERMSHIFT, 1 the portable 40-bit reference (host only), 2 the DSP accumulator (the
 * target default). Printed by ?ta so a log of sink= and the two halves says which
 * arithmetic produced them. */
uint8_t avas_line_ck_acc_mode(void);

/* Present only in a -Define AVAS_TYPE_TY_CK_SATPROBE=1 build: drives `sac` past the
 * 16-bit clamp on purpose, because the engine itself never gets there. Returns
 * +32767 if SATDW saturates a W-register destination, -6 if it wraps. */
 ...
 * when this goes false, so `*tb0003` (gate masked) followed by `*ta0000` left the
 * AVAS stage summing into the chain for good, with no way back short of a restart.
 */
