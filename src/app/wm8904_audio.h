...
/* Sonora-compatible one-shot declick restart for *td<NN>.  The bitmask is
 * active only for this full stop -> configure -> start transition; normal
 * recovery and subsequent *tr commands return to the baseline strategy. */
bool wm8904_audio_restart_declick(uint8_t declick_mask);

/* ?td reports whether this codec has a retained DC-servo sample for the
 * WARM_SERVO strategy. */
 ...
 * DEFAULT 0: the EV88G73A Curiosity Nano drag-and-drop programmer rejects the
 * 98 %-full image that carries these lab probes.  The shipping/default image
 * therefore leaves them out, preserving room for the console compatibility and
 * audio-safety paths.  Opt in with -Define WM8904_AUDIO_ENABLE_TDM_DIAG=1 for
 * a lab build on a programming path that can accept the larger image.
 *
 * WHAT IS GATED, and why these two and not others:
 *
 ...
 * route's ?tq load, ?tp slot mask and miss count on the same hardware from the same
 ...
 *                                   cannot drift apart.
 *
 * There is deliberately no CK-only structured select/start contract: `*cy00` is the
 * Sonora Classic-compatible Type_TY structured toggle, and Type_LB stays on the `A` key.
 */
 ...
 * so switching it is how the 185 lines' contribution is heard.
 *
...
/*
 * AVAS pitch trim, cent (100 cent = 1 semitone), 0 = the voice's own pitch,
 * clamped to +/-AVAS_LINE_CK_PITCH_LIMIT_CENT (avas_synth_line_ck.h). Applies to
 * whichever voice (Type_TY/Type_LB) is currently loaded, since both share the one
 * avas_line_ck_t instance -- see avas_line_ck_request_pitch_cent(). Reset to
 * 0 cent on stop, once the ~3 s fade has actually finished.
 *
 * *tc<NNNN> (console) calls _set(); the up/down arrow hotkeys
 * (ev88g73a_console_out.c) call _step() with +-5. Both print through the same
 * helper, so *tc and the arrow keys report identically.
 */
 ...
 *
 * SET, not toggled, because the shared console forms are `*tq` / `*tq0000` (OFF) and
 * `*tq0001` (ON). A repeated command, or one whose echo was missed, cannot leave the board
 * in the opposite state to the one asked for, which a toggle can. `*tq0002YYYY` is a
 * Sonora-only period-selection extension; the CK command handler recognises it and returns
 * UNSUPPORTED because this board's period is fixed by its profile.
 *
