...
bool wm8904_init( uint8_t inst, bool master_cfg )
{
    ...
    {
        /*
         * CPU reset does not reset the WM8904.  Discharge its analog state before
         * the software reset in wm8904_config().  wm8904_shutdown() is deliberately
         * strategy-aware: *td arms its mask only for the following restart, while
         * the normal boot / *tr path uses the proven WSEQ baseline.  Keep the final
         * quench as well: this is the measured sequence used by Sonora's declick
         * experiments and clears any residual HPOUT state after the ordered shutdown.
         */
