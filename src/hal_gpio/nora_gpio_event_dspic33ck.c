...
typedef struct
{
     ...
     * time base, SPI1 RX/TX and the console UART -- so that store puts back whatever those bits were at the load, erasing
 ...
 * (a read-modify-write on dsPIC33A, where there is no BFINS). Written this way both
 * arms store a
