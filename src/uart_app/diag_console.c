 ...
 * ------------------------------------
 * ?dt tests the high-resolution timer, and that timer is the HAL's
...
/*
 * "?dt" -- prove (or disprove) the high-res timer's 32-bit read.
 *
...
static void console_diag_timer_selftest(void)
{
    ...
    if (!nora_high_res_timer_is_initialized()) {
        console_out_str("?dt: high-res timer not initialized\n");
        return;
    }
    ...
    console_out_str("?dt: SCCP1 read test, ");
