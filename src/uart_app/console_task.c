...
void console_task_print_help(void)
{
    console_out_str(
        ...
        "   ?gv version/build ID  ?gh hello + this help\n"
        "   *sr reset          ?sr reset cause\n"
        "   ?dh high-res timer self-test  ?dt retired (use ?dh)\n"
        "   ?du console RX overruns recovered\n");
...
void console_task_poll(void)
{
     ...
     * out of the middle of a command -- 'a' can be a payload character -- and the
